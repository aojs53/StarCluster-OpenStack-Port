...
class SlurmPlugin(clustersetup.DefaultClusterSetup):
    ...
    packages_installed_file = "/root/.slurm-yum-log"

    # Remote exceptions to check for
    #remote_exceptions = (SSHError, SFTPError, IOError)
    remote_exceptions = (SSHError, IOError)
    ...
    def copy_rpm_file(self,node,movefile=True):
        """
        Copy the local built rpm files that are not available in the RPM reps
        to the desinate host over the ssh session.
        """
        if movefile:
            scp = node.ssh.scp
            from os.path import expanduser
            home = expanduser("~")
            src=home+"/.starcluster/slurm-munge-openmpi-mysqlpatch-0.3.tar"
            scp.put(src,"/home")
            node.ssh.execute("cd /home; tar xvzf /home/slurm-munge-openmpi-mysqlpatch-0.3.tar")

        node.ssh.execute("yum -y install /home/x86_64/*.rpm")
        if movefile:
            #patch the rpm build missing the accounting_storage_mysql.so
            node.ssh.execute("cp /home/x86_64/accounting_storage_mysql.so  /usr/lib64/slurm/")
    ...
    def _force_restart_slurm(self, master):
        ...
        try:
            master.ssh.execute("/etc/init.d/slurm stop",
                               raise_on_failure=True)
            master.ssh.execute("/etc/init.d/slurm start",
                               raise_on_failure=True)
        except self.remote_exceptions, e:
            raise SlurmControllerError(
    ...
    def _configure_munge_master(self, master):
        ...
        try:
            ...
            if not master.ssh.isfile("/etc/munge/munge.key"):
                master.ssh.execute(
    ...
    def _set_slurm_attributes(self, master, nodes):
        ...
        self.slurm_attributes = {
           ...
           "cluster-name": master.cluster_groups[0].name.replace('_sc-', ''),
    ...
    def _start_slurmdbd_master(self, master):
        ...
        try:
            master.ssh.execute("/etc/init.d/slurmdbd stop")
            master.ssh.execute("/etc/init.d/slurmdbd start",
                               raise_on_failure=True)
        except self.remote_exceptions, e:
            raise SlurmControllerError(
                "Unable to restart SLURM controller: " + str(e))
    ...
    def _start_mysql(self, node):
        ...
        try:
            node.ssh.execute("/etc/init.d/mysqld stop")
            node.ssh.execute("/etc/init.d/mysqld start", raise_on_failure=True)
        except self.remote_exceptions, e:
    ...
    def _start_slurm_node(self, node):
        ...
        try:
            node.ssh.execute("/etc/init.d/slurm stop")
            node.ssh.execute("/etc/init.d/slurm start",
                             raise_on_failure=True)
        except self.remote_exceptions, e:
            raise SlurmError(
